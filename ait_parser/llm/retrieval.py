...
class Retriever:
    ...
    def __init__(self, persist_dir: Path, top_k: int = 3,
                 balance_sources: bool = True, runbook_only: bool = False):
        """
        ...
        runbook_only: if True, retrieve ONLY from the runbook source. This
            isolates the effect of the curated, task-relevant operational
            corpus from the noisier CVE/MITRE corpus. Overrides balance_sources.
        """
        self._store = KnowledgeStore(persist_dir)
        self._top_k = top_k
        self._balance = balance_sources
        self._runbook_only = runbook_only
    ...
    def retrieve(self, alert: dict, top_k: Optional[int] = None) -> List[dict]:
        ...
        with self._lock:
            ...
            # Runbook-only mode: retrieve exclusively from the curated
            # operational runbook corpus, ignoring CVE and MITRE entirely.
            if self._runbook_only:
                return self._query_source(query_vec, "runbook", k)
